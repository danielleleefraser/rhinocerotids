...
#saves counts from individual BSMs
library(reshape2)
temp<-counts_list$all_dispersals_counts_cube[,,1]
colnames(temp)<-c("A","E","F","M","N")
rownames(temp)<-c("A","E","F","M","N")
temp2<-melt(temp)
temp2<-cbind(temp2,rep(1,25))
colnames(temp2)[4]<-c("Iteration")
for(b in 2:50){
  temp<-counts_list$all_dispersals_counts_cube[,,b]
  colnames(temp)<-c("A","E","F","M","N")
  rownames(temp)<-c("A","E","F","M","N")
  temp3<-melt(temp)
  temp3<-cbind(temp3,rep(b,25))
  colnames(temp3)[4]<-c("Iteration")
  temp2<-rbind(temp2,temp3)
}

write.csv(temp2,"All counts BSMs.csv")#writes to file, mainly for plotting
