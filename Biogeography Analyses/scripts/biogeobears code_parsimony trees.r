...
setwd(here())
trees<-read.nexus("Dated parsimony trees.nex")
#Use below for the Lu et al trees
#trees<-read.nexus("Dated parsimony trees Lu et al.nex") # The European land mammal age tree

for(k in 1:length(trees)){
  
  setwd(here("data//"))
  trnew<-trees[[k]]
  randtree<-multi2di(trnew, random = TRUE) # this fixes any polytomies
  randtree$edge.length <-  randtree$edge.length + 0.000001 # this fixes any zero length branches
  write.tree(randtree,here("Temp_tree.newick"))
  
  trfn = here("Temp_tree.newick")
  
  tr = read.tree(trfn)
  
  geogfn = here("BioGeoPAtable_new.phy")
  # For Lu tree use
  #geogfn = here("BioGeoPAtable_lu.phy")
  ...
  # Okay need to make the biogeo dist file.
...
# Intitialize a default model (DEC model)
  ...
  # Uncomment files you wish to use in time- analyses:
  #BioGeoBEARS_run_object$timesfn = "timeperiods_21.txt" #use timeperiods_35 for 35 Ma strat analysis
  #BioGeoBEARS_run_object$dispersal_multipliers_fn = "manual_dispersal_multipliers_mio.txt"
  ...
  # Divide the tree up by timeperiods/strata (uncomment this for  analysis)
  #BioGeoBEARS_run_object = section_the_tree(inputs=BioGeoBEARS_run_object,cut_fossils=FALSE,make_master_table=TRUE,plot_pieces=FALSE)
  # The  tree is described in this table:
  ...
  resfn = paste("DEC+j",k,".Rdata",sep="_")
  runslow = TRUE
  if (runslow)
  {
  ...
  # pdffn = "Rhinos_DEC_vs_DEC+J_M0_unconstrained_v1.pdf"
  ...
  #######################################################
  #analysis_titletxt ="BioGeoBEARS DEC+J on Bears M0_unconstrained"
  ...
  runslow = TRUE
  resfn = paste("DIVALIKE",k,".Rdata",sep="_")
  if (runslow)
  {
    res = bears_optim_run(BioGeoBEARS_run_object)
    res    
  ...
  resfn = paste("DIVALIKE+j",k,".Rdata",sep="_")
  runslow = TRUE
  if (runslow)
  {
    #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
  ...
  runslow = TRUE
  resfn = paste("BAYAREALIKE",k,".Rdata",sep="_")
  if (runslow)
  {
    res = bears_optim_run(BioGeoBEARS_run_object)
    res    
  ...
  BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["d","min"] = 0.000000000001
  BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["d","max"] = 19.9999999
  
  BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["e","min"] = 0.0000001
  BioGeoBEARS_run_object$BioGeoBEARS_model_object@params_table["e","max"] = 19.9999999
  ...
  resfn = paste("BAYAREALIKE+j",k,".Rdata",sep="_")
  runslow = TRUE
  if (runslow)
  {
    res = bears_optim_run(BioGeoBEARS_run_object)
  ...
  #pdffn = "Rhinos_BAYAREALIKE_vs_BAYAREALIKE+J_M0_unconstrained_v1.pdf"
  #pdf(pdffn, width=8, height=16)
  
  #######################################################
  # Plot ancestral states - BAYAREALIKE
  #######################################################
  #analysis_titletxt ="BioGeoBEARS BAYAREALIKE on Bears M0_unconstrained"
  ...
  #######################################################
  #analysis_titletxt ="BioGeoBEARS BAYAREALIKE+J on Bears M0_unconstrained"
  ...
  # States
  #res1 = plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="text", label.offset=0.3, tipcex=0.2, statecex=0.3, splitcex=0.2, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)
  
  # Pie chart
  #plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", tipboxes_TF =FALSE,plotlegend=TRUE, legend_cex=1,label.offset=0.3, tipcex=0.3, statecex=0.2, splitcex=0.2, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)
  ...
  #########################################################################
  # The error must be occurring here
  teststable$alt = c("DECj", "DIVALIKEj", "BAYAREALIKEj")
  ...
  # Loads to "restable"
  save(restable, file=paste(k,"restable_v1.Rdata",sep="")) 
  
  
  # Loads to "teststable"
  save(teststable, file=paste(k,"testable_v1.Rdata",sep=""))
  load(file=paste(k,"testable_v1.Rdata",sep=""))
  
  # Also save to text files
  write.table(restable, file=paste(k,"restable.txt",sep=""), quote=FALSE, sep="\t")
  write.table(unlist_df(teststable), file=paste(k,"testable.txt",sep=""), quote=FALSE, sep="\t")
  ...
  # Also save to text files
  write.table(restable_AIC_rellike, file=paste(k,"restable_AIC_rellike.txt",sep=""), quote=FALSE, sep="\t")
  write.table(restable_AICc_rellike, file=paste(k,"restable_AICc_rellike.txt",sep=""), quote=FALSE, sep="\t")
  
  # Save with nice conditional formatting
  write.table(conditional_format_table(restable_AIC_rellike), file=paste(k,"restable_AIC_rellike_formatted.txt",sep=""), quote=FALSE, sep="\t")
  write.table(conditional_format_table(restable_AICc_rellike), file=paste(k,"restable_AICc_rellike_formatted.txt",sep=""), quote=FALSE, sep="\t")
  
  # it's breaking somewhere here for some reason
  bestmodel<-which(restable_AICc_rellike$AICc==min(restable_AICc_rellike$AICc))
  model_name = rownames(restable_AICc_rellike)[bestmodel]
  model_list<-list(resDEC,resDECj,resDIVALIKE,resDIVALIKEj,resBAYAREALIKE,resBAYAREALIKEj)
  res = model_list[[bestmodel]]
  ...
  #
  # nummaps_goal = 50
  # for (k in 1:nummaps_goal)
  # {
  #   clado_events_table = clado_events_tables[[k]]
  #   analysis_titletxt = paste0(model_name, " - Stochastic Map #", k, "/", nummaps_goal)
  ...
  outfn = paste0("SUMMARY all_dispersals_counts_fromto_means",k,".txt",sep="")
  write.table(counts_list$all_dispersals_counts_fromto_means, file=outfn, quote=FALSE, sep="\t", col.names=TRUE, row.names=TRUE)
  
  #counts_list$all_dispersals_counts_fromto_sds
  outfn = paste0("SUMMARY all_dispersals_counts_fromto_sds",k,".txt",sep="")
  write.table(counts_list$all_dispersals_counts_fromto_sds, file=outfn, quote=FALSE, sep="\t", col.names=TRUE, row.names=TRUE)
  
  # We need to write this. 
  
  summary_counts_BSMs = counts_list$summary_counts_BSMs
  print(conditional_format_table(summary_counts_BSMs)) # write this as well
  outfn = paste0("SUMMARY counts_BSMs",k,".txt",sep="")
  write.table(summary_counts_BSMs, file=outfn, quote=FALSE, sep="\t", col.names=TRUE, row.names=TRUE)
  ...
  {
    cmdtxt = paste0("item = counts_list$", tmpnames[j]) #herehere
    ...
    outfn = paste0(tmpnames[j],k,".txt",sep="")#HERE HERE HERE
    if (length(item) == 0)
    {
